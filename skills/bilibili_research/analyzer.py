...
def _create_analysis_prompt(
    ...
    """Create the analysis prompt with adaptive structure based on content type."""
    if enhanced:
        return f"""请分析以下视频/音频转录文本。

**第一步：识别内容类型**
首先判断视频的内容类型，然后根据类型生成适配的报告结构。

常见内容类型：
- 公司/商业分析：公司研究、行业分析、商业模式
- 技术/教程：编程教学、技术讲解、操作指南
- 评论/观点：时事评论、观点表达、价值判断
- 新闻/资讯：新闻报道、资讯汇总、事件梳理
- 访谈/对话：人物访谈、对话讨论
- 其他：以上类型之外的任何内容
...
根据内容类型补充提取：
- 公司/商业分析 → 财务数据、市场数据、竞争对比、风险机会
- 技术/教程 → 技术要点、步骤说明、代码示例、注意事项
- 评论/观点 → 主要论点、论据支持、反驳观点、结论判断
- 新闻/资讯 → 事件要素、时间线、相关方、影响分析
- 访谈/对话 → 对话要点、双方观点、共识分歧、核心结论

**第三步：生成详细内容**

将上述提取的信息，组织成结构化的详细章节。章节标题和数量应该根据内容类型自适应调整，不要使用固定的"技术/概念详解"、"数据汇总"、"对比分析"等模板。

示例：
- 公司分析可能是：公司概况、财务表现、产品技术、竞争格局、风险机会
- 技术教程可能是：背景介绍、核心概念、实现步骤、注意事项、延伸阅读
- 评论观点可能是：事件背景、各方观点、分析论证、结论判断
...
✅ 正确示例：
- "XX公司预计于2024年5月中旬上市，估值约351亿美元，代码为CBRS。"
- "该技术采用晶圆级芯片（WSE）路线，面积约为Nvidia H100 GPU的56倍。"

**第四步：提取数据**

提取视频中提到的所有数值数据，包括标签、数值、单位、分类和上下文。
...
请以JSON格式返回，格式如下：
{{
  "content_type": "内容类型（company/tech/tutorial/opinion/news/interview/other）",
  "outline": ["要点1", "要点2", ...],
  "core_points": ["观点1", "观点2", ...],
  "key_entities": ["实体1", "实体2", ...],
  "verifiable_claims": ["可验证陈述1", "可验证陈述2", ...],
  "keywords": ["关键词1", "关键词2", ...],
  "summary": "一句话总结",
  "detailed_sections": [
    {{
      "heading": "根据内容自适应的章节标题",
      "content": "详细内容段落1。\\n\\n详细内容段落2。\\n\\n详细内容段落3。",
      "timestamps": [0, 120]
    }}
  ],
